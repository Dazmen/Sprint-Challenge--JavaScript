...
function sortUni(uni){
  return uni.sort(function(a, b){
...
const contactInfo = function(graduates){
  const info = [];
...
const unisWithUni = function(graduates){
  ...
  for (let i = 0; i < graduates.length; i++){
    if(graduates[i].university.includes('Uni')){
      info.push(graduates[i].university);
    }
  }return info;
};
...
*/
const displayNames = function(zooAnimals){
  const info = [];
  zooAnimals.forEach(function(item, index, array){
    info.push(`Name: ${item.animal_name}, Scientific: ${item.scientific_name}`)
  });
  return info;
}
...
const lowCaseAnimalNames = function(zooAnimals){
  const info = [];
  zooAnimals.map(function(item, index, array){
    info.push(`${item.animal_name}`)
  })
  return info;
};
